...
class ProcessingStatus(models.TextChoices):
    """Allowed processing states for an `XRayImage`.

    Stored values are stable API/database identifiers.
    """
...
class XRayImage(models.Model):
    ...
    # Keep the default FK index: it helps hospital scoping joins and cascading deletes.
    user = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name='xray_images', null=True)
    
    # Patient Information
    # Not indexed: UI/admin uses substring searches (icontains) which won't use btree
    # indexes; keep inserts/updates faster by avoiding low-value indexes.
    first_name = models.CharField(max_length=100, blank=True)
    last_name = models.CharField(max_length=100, blank=True)
    patient_id = models.CharField(max_length=100, blank=True)
    # Indexed for fast age filters in history views.
    ...
    # Not indexed: rarely filtered; keeps writes cheaper.
    technologist_first_name = models.CharField(max_length=100, blank=True)
    technologist_last_name = models.CharField(max_length=100, blank=True)
    ...
    # Not indexed: boolean columns are low-selectivity and not used in hot queries.
    ...
    #
    # Performance note:
    # These columns are updated on every inference run. Indexing every probability
    # field causes heavy write amplification (many index updates per row) while the
    # application primarily filters/sorts on `severity_level`, timestamps, and
    # categorical fields. We intentionally keep these unindexed.
    ...
    model_used = models.CharField(max_length=50, default='densenet')
    ...
    class Meta:
        constraints = [
            # Defensive data integrity: progress must stay within 0-100.
            models.CheckConstraint(
                check=models.Q(progress__gte=0, progress__lte=100),
                name="xrayimage_progress_range",
            ),
            # MTS triage levels are 1..5 (or NULL when unknown).
            models.CheckConstraint(
                check=(
                    models.Q(severity_level__isnull=True)
                    | models.Q(severity_level__gte=1, severity_level__lte=5)
                ),
                name="xrayimage_severity_range",
            ),
        ]
...
class PredictionHistory(models.Model):
    ...
    xray = models.ForeignKey(XRayImage, on_delete=models.CASCADE, related_name='prediction_history', db_index=False)
    ...
    class Meta:
        ...
        indexes = [
            # Optimize `update_latest_for_xray()` (filter by xray, order by newest).
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                name="predhistory_severity_range",
            ),
        ]
        # Optimize database table order
        ordering = ['-created_at']
...
class VisualizationResult(models.Model):
    ...
    # FK index is redundant (we have multiple composite indexes starting with xray).
    xray = models.ForeignKey(XRayImage, on_delete=models.CASCADE, related_name='visualizations', db_index=False)
    
    # Visualization details
    # Not indexed individually: all hot queries filter by `xray` and use the
    # composite indexes in `Meta.indexes`.
    visualization_type = models.CharField(max_length=30, choices=VISUALIZATION_TYPES, db_index=False)
    target_pathology = models.CharField(max_length=50, db_index=False)  # The pathology this visualization targets
    ...
    class Meta:
        ...
        indexes = [
            # Optimize "all visualizations for an xray, newest first".
            models.Index(fields=['xray', '-created_at']),
        ]
...
class SavedRecord(models.Model):
    ...
    # FK indexes are redundant (we have composite/unique indexes starting with these FKs).
    user = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name='saved_records', db_index=False)
    prediction_history = models.ForeignKey(PredictionHistory, on_delete=models.CASCADE, related_name='saved_by_users', db_index=False)
    # Indexed via the composite (user, saved_at) / (prediction_history, saved_at) indexes below.
